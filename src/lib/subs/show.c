...
show_nuke_build(int tlev)
{
	...
	for (np = nchr, n = 0; n < N_MAXNUKE; np++, n++) {
		avail = (4 + np->n_rad + np->n_oil + np->n_lcm + np->n_hcm * 2)/5;
		if (np->n_tech > tlev) 
			continue;
...
show_sect_capab(int foo)
{
	...
	for(x = 0; x < SCT_MAXDEF + 2; x++) {
		...
		pr("%c %-23s %-7s ", dchr[x].d_mnem, dchr[x].d_name, pchr[j].p_sname);
