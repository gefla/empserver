...
defense_val(struct lndstr *lp)
{
	...
	value *= ((double)land_mob_max + lp->lnd_harden) / (double)land_mob_max;
	value = (int)((double)value * ((double)lp->lnd_effic / 100.0));
   	    		...
   	    		/* number of casualties to take */
{
	...
	if (llp->lcp->l_flags & L_SPY) {
	    ...
	    eff_eq = ldround((((double)cas*100.0) / (double)llp->lcp->l_mil),1);
...
int
intelligence_report(int destination, struct lndstr *lp, int spy, s_char *mess)
{
	struct	lchrstr *lcp;
	...
	if (chance((double)(spy+lp->lnd_vis)/10.0)){
		...
		if (chance((double)(spy+lp->lnd_vis)/20.0)){
			...
			if (destination == player->cnum)
				pr(" (eff %d, mil %d", roundintby(lp->lnd_effic,5), roundintby(lnd_getmil(lp),10));
			else
				sprintf(buf2," (eff %d, mil %d", roundintby(lp->lnd_effic,5), roundintby(lnd_getmil(lp),10));
...
void
lnd_mar(struct emp_qelem *list, double *minmobp, double *maxmobp, int *togetherp, natid actor)
{
	struct	emp_qelem *qp;
...
void
lnd_sweep(struct emp_qelem *land_list, int verbose, int takemob, natid actor)
{
	struct  emp_qelem   *qp;
	struct  emp_qelem   *next;
	struct  llist   *llp;
...
static int lnd_easiest_target(struct emp_qelem *list)
{
	struct	emp_qelem *qp;
	struct	emp_qelem *next;
	struct	llist *llp;
...
static int
lnd_missile_interdiction(struct emp_qelem *list, coord newx, coord newy, natid victim)
{
        int     dam;
...
int
lnd_mar_one_sector(struct emp_qelem *list, int dir, natid actor, int together)
{
	...
	for (qp=list->q_back;qp!=list;qp=next) {
		...
		if (!(lchr[(int)llp->land.lnd_type].l_flags & L_SPY)) visible = 1;
		if (sect.sct_rail == 0 &&
		    lchr[(int)llp->land.lnd_type].l_flags & L_TRAIN) {
			if (together) {
	...
	if (QEMPTY(list))
		return stopping;
	if (visible) stopping |= lnd_interdict(list, newx, newy, actor);
	return stopping;
}
