...
int pinflak_planedamage(struct plnstr *pp, struct plchrstr *pcp, natid from, int flak);
...
bomb(void)
{
	...
	if (!snxtitem(&ni_esc, EF_PLANE, getstarg(player->argp[2], "escort(s)? ", buf)))
		pr("No escorts...\n");
	if ((p = getstarg(player->argp[3], "pinpoint, or strategic? ", buf)) == 0)
		return RET_SYN;
	mission = *p;
	if (index("ps", mission) == 0)
		return RET_SYN;
	if ((p = getstarg(player->argp[4], "assembly point? ", buf)) == 0 || *p == 0)
		return RET_SYN;
	...
	mission_flags = pln_arm(&bomb_list, 2*ap_to_target, mission, ip, 0, mission_flags, &tech);
	if (QEMPTY(&bomb_list)) {
		pr("No planes could be equipped for the mission.\n");
		return RET_FAIL;
	}
	mission_flags = pln_arm(&esc_list, 2*ap_to_target, mission, ip,P_F|P_ESC,mission_flags,&tech);
...
retry:
	p = getstring("Bomb what? (ship, plane, land unit, efficiency, commodities) ", buf);
	if (p == 0 || *p == 0) {
		if (player->aborted)
			return;
		bad++;
...
eff_bomb(struct emp_qelem *list, struct sctstr *target)
{
	...
	for (qp = list->q_forw; qp != list; qp = qp->q_forw) {
		...
		if (plp->bombs || plp->plane.pln_nuketype != -1)
			dam += pln_damage(&plp->plane, target->sct_x, target->sct_y, 'p', &nukedam, 1);
	}
...
comm_bomb(struct emp_qelem *list, struct sctstr *target)
{
	...
	putvar(ip->i_vtype, commdamage(amt, dam, ip->i_vtype), (s_char *)target, EF_SECTOR);
...
ship_bomb(struct emp_qelem *list, struct sctstr *target)
{
	...
	for (qp = list->q_forw; qp != list && !player->aborted; qp = qp->q_forw) {
		if (head.uid != -1){
			s=head.next;
		...
		if (plp->plane.pln_nuketype != -1)
			...
			hitchance = pln_hitchance(&plp->plane, shp_hardtarget(&ship), EF_SHIP);
			pr("%d%% hitchance...", hitchance);
		}
		if (roll(100) <= hitchance) {
			/* pinbombing is more accurate than normal bombing */
			dam = 2 * pln_damage(&plp->plane, target->sct_x, target->sct_y, 'p', &nukedam, 1);
		} else {
			pr("splash\n");
			/* Bombs that miss have to land somewhere! */
			dam = pln_damage(&plp->plane, target->sct_x, target->sct_y, 'p', &nukedam, 0);
			collateral_damage(target->sct_x, target->sct_y, dam, list);
			dam = 0;
		}
		if (dam <= 0) /* dam == 0 if only nukes were delivered */
	...
	for (qp = list->q_forw; qp != list; qp = qp->q_forw) {
		...
		while (unitno < 0 && !player->aborted && !ignore) {
			...
			if (n < 0)
				continue;
			if (getland(n, &land) &&
			    land.lnd_x == target->sct_x &&
			    land.lnd_y == target->sct_y)
				unitno = n;
			else
		...
		if (flak) {
		    sprintf(msg, "Flak! Firing flak guns from unit %s (aa rating %d)\n",
		...
		if (plp->plane.pln_nuketype != -1)
			...
			hitchance = pln_hitchance(&plp->plane, lnd_hardtarget(&land), EF_LAND);
			pr("%d%% hitchance...", hitchance);
		}
		if (roll(100) <= hitchance) {
			dam = 2 * pln_damage(&plp->plane,target->sct_x,target->sct_y, 'p', &nukedam, 1);
...
strat_bomb(struct emp_qelem *list, struct sctstr *target)
{
	...
	for (qp = list->q_forw; qp != list; qp = qp->q_forw) {
		...
		if ((plp->pcp->pl_flags & P_C) && (!(plp->pcp->pl_flags & P_T)))
			continue;
		if (plp->bombs || plp->plane.pln_nuketype != -1)
			dam += pln_damage(&plp->plane, target->sct_x, target->sct_y, 's', &nukedam, 1);
	}
	if (dam <= 0) /* dam == 0 if only nukes were delivered */
		return;
	getsect(target->sct_x, target->sct_y, &sect);
...
int
pinflak_planedamage(struct plnstr *pp, struct plchrstr *pcp, natid from, int flak)
{
