...
laun(void)
{
	...
	while (nxtitem(&nstr, (s_char *)&plane)) {
		...
		if (plane.pln_ship >= 0) {
			...
			rel = getrel(natp, player->cnum);
			if (sect.sct_own && sect.sct_own != player->cnum && rel != ALLIED) {
...
launch_as(struct plnstr *pp)
{
	...
	if (msl_hit(pp, plane.pln_def, EF_PLANE, N_SAT_KILL, N_SAT_KILL, prplane(&plane), sx, sy, plane.pln_own)) {
...
launch_missile(struct plnstr *pp, int sublaunch)
{
	...
	{ /* else */ 
		...
		dam = pln_damage(pp,target_ship.shp_x,target_ship.shp_y,'p',&nukedam, 1);
		if (!nukedam) {
...
launch_sat(struct plnstr *pp, int sublaunch)
{
	...
	pr("%s positioned over %s", prplane(pp),
	   xyas(sx, sy, player->cnum));
