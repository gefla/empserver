...
static void noise(struct sctstr *sptr, int public_amt, s_char *name, int old, int new);
static void benefit(natid who, int good);
int doland(s_char op, int arg, struct sctstr *sect);
int docountry(s_char op, int arg, s_char *p, float farg, natid nat, struct natstr *np);
...
doland(s_char op, int arg, struct sctstr *sect)
{
	...
	case 'R':
		...
		if (arg < 0)
		  arg = 0;
		noise(sect, 1, "Road percentage", (int) sect->sct_road,
			arg);
		sect->sct_road = arg;
		break;
	case 'r':
		...
		if (arg < 0)
		  arg = 0;
		noise(sect, 1, "Rail percentage", (int) sect->sct_rail,
			arg);
		sect->sct_rail = arg;
		break;
	case 'd':
		...
		if (arg < 0)
		  arg = 0;
		noise(sect, 1, "Defense percentage", (int) sect->sct_defense,
			arg);
...
int
docountry(s_char op, int arg, s_char *p, float farg, natid nat, struct natstr *np)
{
	extern  int m_m_p_d;
	coord	newx, newy;
	switch (op) {
