...
static
s_char *
lowerit(s_char *buf, int n, s_char *orig) /* converts a string to lower case */
     ...
     /* directory entry pointer pointer */
{
	...
	d = (struct dirent *)malloc(sizeof(struct dirent) + pathconf(".", _PC_NAME_MAX) +1);
	...
	*dpp = (struct dirent **)malloc(nsize * sizeof(struct dirent *));
	...
	while ( (d = readdir_r(dirp, d)) != NULL)
		...
		if ( select != NULL && !(*select)(d) )
			continue;
		tmp = (struct dirent *)malloc(sizeof(struct dirent) + strlen(d->d_name) + 1);
		if ( tmp == NULL )
			return -1;
		tmp->d_ino = d->d_ino;
		...
		if ( nents > nsize )
			{
			*dpp = (struct dirent **)realloc((char *)*dpp, nsize * sizeof(struct dirent *));
			if ( *dpp == NULL )
				return -1;
			}
		(*dpp)[nents++] = tmp;
...
printdir(void)
{
	...
	extern	int alphasort(const struct dirent *const *, const struct dirent *const *);
#endif
	extern	s_char *infodir;
